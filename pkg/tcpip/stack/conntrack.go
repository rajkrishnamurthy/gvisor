...
func (ct *ConnTrack) connFor(pkt *PacketBuffer) (*conn, direction) {
	...
	if tuple.conn.timedOut(time.Now()) {
		delete(ct.conns, tuple.id())
		delete(ct.conns, tuple.conn.reply.id())
		return nil, dirOriginal
	}
...
func (ct *ConnTrack) HandlePacket(pkt *PacketBuffer, hook Hook, gso *GSO, r *Route) {
	...
	// Mark the connection as having been used recently so it isn't garbage
	// collected.
...
// collectGarbage deletes timed out entries from the conntrack map.
// TODO(gvisor.dev/issue/170): This can be more finely controlled, as it is in
// Linux via sysctl.
func (ct *ConnTrack) collectGarbage() {
	now := time.Now()
	...
	var deadConns []connID
	for id, tuple := range ct.conns {
